...
-define(attribute(Name,Value),?attribute_(Name,[?abstract(Value)])).
-define(attribute_(Name,Values),erl_syntax:attribute(?atom(Name),Values)).
-define(export(Fun,Arity),?attribute_(export,[?list([erl_syntax:arity_qualifier(?atom(Fun),?int(Arity))])])).
